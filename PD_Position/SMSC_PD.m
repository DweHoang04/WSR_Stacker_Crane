...
kp1 = 2.5; kd1 = 10; % Hệ số 2 khâu PD cho xe con
kp2 = 4; kd2 = 10; % Hệ số 2 khâu PD cho xe nâng
...
%--------------------------------------------------------------------------
    % Xe con
    e1(j + 1) = sp1 - w(1,j);
    D1 = (e1(j + 1) - e1(j))/delta_t;
    F1(j + 2) = kp1*e1(j + 1) + kd1*D1;
    if F1(j + 2) > 15
        F1(j + 2) = 15;
    elseif F1(j + 2) < -15
        F1(j + 2) = -15;
    ...
    % Xe nâng
    e2(j + 1) = sp2 - dx2dt_2(j);
    D2 = (e2(j + 1) - e2(j - 1))/(delta_t*2);
    F2(j + 2) = kp2*e2(j + 1) + kd2*D2;
%     if F2(j + 2) > 15
%         F2(j + 2) = 15;
%     elseif F2(j + 2) < -15
%         F2(j + 2) = -15;
