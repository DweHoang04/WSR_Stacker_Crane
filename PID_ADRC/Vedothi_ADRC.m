...
hold on 
grid on 
plot(out.tout,out.X1,'color', [0.07,0.62,1.00],'linewidth',2);
...
grid on 
plot(out.tout,out.X1,'color', [0.07,0.62,1.00],'linewidth',2);
axis([4 6 0.196 0.204]);
yticks([0.196 0.2 0.204]);
xticks([4 5 6]);
...
plot(freq,abs_w(1:length(freq)),'color', [0.07,0.62,1.00],'linewidth',1.5); 

fft_w=fft(out.W_MID,l1)*(2/l1);
abs_w=abs(fft_w);
freq=0:(1/time(end)):Fs/2-(1/time(end));
plot(freq,abs_w(1:length(freq)), '--r', 'LineWidth',2); 
xlabel('Tần số (Hz)');
ylabel('Biên độ (m)')
legend('Đỉnh thang','Điểm giữa thang');
A = [0 10 0 0.02 ];
